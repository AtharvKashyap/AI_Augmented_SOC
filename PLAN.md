...
- [x] Create `tests/fixtures/labeled/` with 40+ alerts and candidates — **42 cases**: 5 seed, 12 benign, 12 ambiguous, 13 serious, each with its own replay fixture. Every label was written from security reasoning *before* the scorer was run, and where the scorer disagreed the label was kept and the disagreement investigated — never the reverse, since labeling to match the scorer is what makes an evaluation tautological.
  ...
  - Current: **in-band 95.2%, action agreement 100%, mean score error 0.048, benign-caught 100%**, zero missed true positives, zero noise cases.
  - **Read those numbers with suspicion.** They are agreement with labels we wrote ourselves, which is what overfitting looks like. The defence is that every fix reads a real telemetry field with a stated rationale — `data.win.eventdata.logonType`, `rule.firedtimes`, `network.bytes` — rather than pattern-matching the test data. That is an argument, not proof. Analyst-reviewed labels are the only thing that settles it.
  - Two cases remain out of band by one point, **both with the correct action**. They are recorded under Known defects → Open as stated capability limits, not tuned away.
...
| **Windows logon type was never extracted, so RDP looked like a file-share access** | Windows logs every successful logon at the same rule level, so the level cannot separate an interactive session on a server from a network logon. `data.win.eventdata.logonType` was in the payload and unread; a successful RDP into production scored 2 and was marked likely-benign. Now extracted and scored (10 RemoteInteractive, 2 Interactive). |
| **Wazuh's own `rule.firedtimes` baseline was unread** | Wazuh counts how often each rule has fired on the deployment — a baseline the product computes for us. Expected file-integrity churn in a log directory is a real level-7 rule *and* noise, and the scorer could not tell. Now damps the score, and **never for high or critical severity**: common is not harmless, and brute-force rules fire constantly. |
| **Transfer volume was never extracted** | Without `network.bytes` / `zeek.conn.orig_bytes`, an 8.79 GB egress and a 9 KB one were the same event to the scorer, carrying the same rule and addresses. |
| **`powershell` was treated as a medium-risk term** | It is the default shell on Windows, so its presence is not a signal — a developer running a setup script scored like a loader. Removed; `encodedcommand`, `-enc`, `rundll32`, `regsvr32` and `mshta` remain, because each indicates an attempt to obscure what is running. |
| **`DEDUP_STORE` accepted values it did not implement** | `DEDUP_STORE=redis` silently got SQLite dedup. Now validated at load time against the one implemented store, failing with a message naming the variable and why Redis is absent. Load time rather than an opt-in validator because dedup is not optional — every run uses it, so there is no later path on which a bad value would surface. `REDIS_URL` removed entirely: with no Redis store every value is wrong, so there is nothing to police. |
| **An evaluation fixture spanning over 30 minutes was silently half-scored** | `_build_triage_scorer` scored `candidates[0]` only, so a case whose events fell outside `ClusteringConfig.time_window_minutes` split and the rest of the evidence vanished — the harness we rely on for the truth was quietly grading a fraction. Now every candidate is scored, the max is taken, and the split is **counted and reported** in `split_cases` plus a stderr note, because "we evaluated half of it" must never look like "we evaluated it". |
| **Milestone 3.5's audit requirement was unmet** | `TriageResult` now carries `enrichment_providers` and `enriched_at`, populated on the local *and* model paths and persisted as queryable columns (in `_ADDED_COLUMNS` too, so existing databases gain them). An empty list with a null timestamp means no enrichment applied — deliberately distinct from enrichment applying and finding nothing. |

### Open

No known incorrect behaviour is outstanding. What remains are **stated capability limits**, not defects — each is a thing the deterministic scorer cannot know, recorded so nobody mistakes it for something that works.

| Limit | Location | Why it is not being "fixed" |
|---|---|---|
| No time-of-day awareness | `soc/triage.py` | `ambiguous-off-hours-login-success` scores 2 against a 3-5 band. "Unusual hour" means unusual *for that account*, which needs a per-user baseline the system does not keep. A naive business-hours window would misfire on any estate spanning timezones — worse than no signal. The action is already correct; only the score is one point low. |
| Cluster bonuses stack above the labeled band on a repeated-failure burst | `soc/triage.py` `score_candidate_locally` | `ambiguous-internal-ssh-failed-burst` scores 7 against a 4-6 band. The composite-rule double count is fixed; what remains is the `primary_user` and multi-address bonuses both applying. The action is correct (`queue_review`), and tuning these against synthetic labels we wrote ourselves is how an evaluation becomes decorative. Tune with real data. |
| `firedtimes` damping is a deployment baseline, not a per-host one | `soc/triage.py` `_frequency_damping` | Wazuh's count is estate-wide, so a rule that is routine on 500 servers and unprecedented on one domain controller reads as routine for both. Per-host baselining needs history the store does not keep. |
| Volume thresholds are magnitudes, not baselines | `soc/triage.py` `VOLUME_BOOST_THRESHOLDS` | Deliberately coarse constants separating "a file" from "a filesystem". A deployment with per-host transfer baselines should replace them with a comparison against that baseline. |
