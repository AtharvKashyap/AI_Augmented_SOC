...
- **The cluster-size bonus counts distinct detections, not alerts.** A Wazuh composite rule like 5720 "Multiple authentication failures" is a summary of the individual failures clustered beside it, so counting alerts scored the aggregate and its own members — one expired service credential became a page. Three of the same rule is one signal; three different rules still earn the bonus.
- **Local candidate scoring reads `asset_context`.** `unknown` never raises a score, because `soc/assets.py` maps both an absent row and an unrecognized value to `unknown` and a gap in the inventory must not read as escalation. The boost is capped below paging reach on its own (`MAX_ASSET_BOOST`): asset context amplifies evidence rather than being evidence, and important hosts are exactly where routine events happen.
...
- **`NON_ESCALATING_RISK_FACTORS` is enforced against the `severity_hint` too, not just the raw factors.** Subtracting it only from `risk_factors` left the `severity_hint == "medium"` disjunct short-circuiting ahead of the guard, so any provider could bypass the invariant by setting a hint — which `LocalEnricher` did, deriving `medium` from `public_ip` or `hash_observable` alone. The set now includes `public_ip` (the mirror of `private_ip`: `score_alert_locally` already adds a point for a non-private `dst_ip`, so boosting again counted one property twice) and `hash_observable` (a hash existing is not evidence about it — there is no reputation lookup behind that tag).
...
- **The labeled set is 42 cases and it earns its keep.** It has now found five defects no unit test caught, two of them repo-wide: Windows Wazuh alerts lost `user`/`process_name`/`command_line` because the paths lacked the `data.` prefix, and every Suricata alert scored three points low because severity lives at `suricata.eve.alert.severity`. Both were silent — the fields are optional and an unknown severity has a valid base score. **When you change scoring, normalization, or a prompt, run `run_eval.py` and read the out-of-band list**; a green test suite does not tell you the answers got worse.
- **Scoring reads three fields beyond the rule level, each because the level cannot distinguish what matters.** `logon_type` (Windows logs every successful logon at the same level, so only the type separates an RDP session on a server from a file-share access), `fired_times` (Wazuh's own per-rule deployment count — a rule that has fired thousands of times describes that estate's background), and `bytes_transferred` (without volume an 8.79 GB egress and a 9 KB one are the same event). All three were already in the payloads and simply unread. `_frequency_damping` **never damps a high or critical alert**: common is not harmless, and brute-force rules fire constantly — there is a test pinning that.
- **`powershell` is deliberately not a medium-risk term.** It is the default shell on Windows, so its presence is not a signal; `encodedcommand`, `-enc`, `rundll32`, `regsvr32` and `mshta` are, because each indicates an attempt to obscure what is running. Do not re-add the interpreter name.
- **Two cases are deliberately left out of band**, recorded in PLAN.md → Known defects → Open rather than tuned away. Both have the correct action and are one point outside their band. They mark stated capability limits — no per-account time-of-day baseline, and cluster bonuses stacking on a repeated-failure burst — not incorrect behaviour. Closing them by tuning against labels we wrote ourselves is the same tautology as labeling from the scorer.
