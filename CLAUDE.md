...
- **Six cases are deliberately left out of band**, recorded in PLAN.md → Known defects → Open rather than tuned away. They mark real blind spots in a keyword-and-severity heuristic: no logon-type awareness (a successful RDP into production from a novel subnet scores 2 and is marked likely-benign), no baseline or volume awareness, and no way for keyword matching to judge context. Closing them by adding keywords would be fitting the scorer to labels we wrote ourselves, which is the same tautology as labeling from the scorer.
